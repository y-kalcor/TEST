...
const char* GetTaskNameFromTask(DWORD *task)
{
	...
	return szTaskNames[(unsigned)GetTaskTypeFromTask(task)];
}
...
void GameDebugDrawTaskInfo()
{
	...
	PrintInfoForTask("PhysicalResp",actor->Tasks->pdwDamage);
	PrintInfoForTask("EventRespTmp",actor->Tasks->pdwFallEnterExit);
	PrintInfoForTask("EventResp",actor->Tasks->pdwSwimWasted);
	PrintInfoForTask("Primary",actor->Tasks->pdwJumpJetPack);
	PrintInfoForTask("Default",actor->Tasks->pdwAction);
	GameDebugDrawNextLine("",D3DCOLOR_ARGB(255,255,255,255));
...
void GameDebugDrawActorInfo()
{
	...
	{		
		...
		_itoa(actor->entity.dwProcessingFlags,s,2);
		sprintf(line_buffer,"ProcFlags: %s",s);
		GameDebugDrawNextLine(line_buffer,D3DCOLOR_ARGB(150,255,255,255));
...
void GameDebugDrawVehicleInfo()
{
	...
	if(vehicle)
	{
		...
		int iModelSlotsAvailable=0;
		BYTE *pModelInfo = GetModelInfo(vehicle->entity.nModelIndex);
		DWORD ModelInfoVehicleStruct = *(DWORD *)(pModelInfo+92);
		DWORD Unk60 = *(DWORD *)(pModelInfo+60);

		_asm mov ebx, 0xB4E680
		_asm mov edx, [ebx]
		_asm mov ebx, [edx+8]
		_asm mov iModelSlotsAvailable, ebx

		sprintf(line_buffer,"mslots: %d modelinf: 0x%X vstruct: 0x%X unk60: 0x%X frlim: %u",
			iModelSlotsAvailable,pModelInfo,ModelInfoVehicleStruct,Unk60,*(PBYTE)0xBA6794);
		...
		sprintf(line_buffer,"MaxPassengers: %u",vehicle->byteMaxPassengers);
		GameDebugDrawNextLine(line_buffer,D3DCOLOR_ARGB(150,255,255,255));
		
		sprintf(line_buffer,"FrontVehicle: 0x%X BackVehicle: 0x%X",vehicle->VehicleAttachedFront,vehicle->VehicleAttachedBottom);
		GameDebugDrawNextLine(line_buffer,D3DCOLOR_ARGB(150,255,255,255));

        VECTOR vecCrossProductResult;
		VECTOR vecMult3x3Inv;
		PED_TYPE *pPlayerPed = GamePool_FindPlayerPed();

		_CrossProduct(&vecCrossProductResult,&pPlayerPed->entity.mat->pos,&vehicle->entity.mat->pos);
		_Multiply3x3(&vecMult3x3Inv,&pPlayerPed->entity.mat->pos,vehicle->entity.mat);
		//_VectorNormalise(&vecCrossProductResult);
		//_VectorNormalise(&vecMult3x3Inv);

		sprintf(line_buffer,"_CrossProductToPlayer: %f %f %f Multi3x3Inv: %f %f %f",
			vecCrossProductResult.X, vecCrossProductResult.Y, vecCrossProductResult.Z,
			vecMult3x3Inv.X, vecMult3x3Inv.Y, vecMult3x3Inv.Z);
		...
		_asm mov w224, ax
			...
			sprintf(line_buffer,"164:%u 165:%u 166:%u 167:%u 168:%u 169:%u 170:%u Ped:0x%X 128:%u AudioEnt:0x%X 220:%u 224:%u",
		byte164,byte165,byte166,byte167,byte168,byte169,byte170,*(DWORD *)0xB6B990,byte128,dwAudioEnt,w220,w224);
		GameDebugDrawNextLine(line_buffer,D3DCOLOR_ARGB(150,255,255,255));

		// Vehicle damage stuff
		sprintf(line_buffer,"dwDamBase: %X EngineStat: %X WheelStat: %u,%u,%u,%u",
			vehicle->dwDamageBase,vehicle->byteEngineStatus,
			vehicle->bCarWheelPopped[0],vehicle->bCarWheelPopped[1],
			vehicle->bCarWheelPopped[2],vehicle->bCarWheelPopped[3]);
		GameDebugDrawNextLine(line_buffer,D3DCOLOR_ARGB(150,200,255,200));

		sprintf(line_buffer,"Doors1: %X Doors2: %X Lights: %X Panel: %X",
			vehicle->dwDoorStatus1,vehicle->wDoorStatus2,vehicle->dwLightStatus,vehicle->dwPanelStatus);
		GameDebugDrawNextLine(line_buffer,D3DCOLOR_ARGB(150,200,255,200));

		sprintf(line_buffer,"Flags: 0x%X",vehicle->byteFlags);
		GameDebugDrawNextLine(line_buffer,D3DCOLOR_ARGB(150,255,255,255));

		sprintf(line_buffer,"Steer1: %f Steer2: %f",vehicle->fSteerAngle1, vehicle->fSteerAngle2);
		GameDebugDrawNextLine(line_buffer,D3DCOLOR_ARGB(150,255,255,255));
					
		sprintf(line_buffer,"Accel: %f Brake: %f",vehicle->fAcceleratorPedal,vehicle->fBrakePedal);
		GameDebugDrawNextLine(line_buffer,D3DCOLOR_ARGB(150,255,255,255));
		
		sprintf(line_buffer,"Hydra: %u",vehicle->dwHydraThrusters);
		GameDebugDrawNextLine(line_buffer,D3DCOLOR_ARGB(150,255,255,255));

		sprintf(line_buffer,"Color1: %u Color2: %u",vehicle->byteColor1,vehicle->byteColor2);
		GameDebugDrawNextLine(line_buffer,D3DCOLOR_ARGB(150,255,255,255));
		
		sprintf(line_buffer,"Siren On: %u, Siren On Alt: %d, Landing Gear: %f", vehicle->bSirenOn, vehicle->dwSirenOnAlt, vehicle->fPlaneLandingGear);
		GameDebugDrawNextLine(line_buffer,D3DCOLOR_ARGB(150,255,255,255));
	}	
}
