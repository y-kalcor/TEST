	...
	if(DLL_PROCESS_ATTACH==fdwReason)
	{
		...
		if(tSettings.bDebug || tSettings.bPlayOnline) {
			...
			if(tSettings.bPlayOnline) {
				...
				if (!pFileSystem->Load(ARCHIVE_FILE)) __asm int 3

				pACGhost = new CACGhost();
				pAntiCheat = new CAntiCheat();
...
void SetupGameUI()
{
	...
	pGameUI->AddEditBox(3,"this is a test box",20,pGame->GetScreenHeight()-40,pGame->GetScreenWidth()-40,36,true);
