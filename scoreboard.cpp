...
void CScoreBoard::Draw()
{
	...
	int localplayer = pPlayerPool->GetLocalPlayerID();
	...
	for (x=m_iOffset; x<endplayer; x++)
	{
		...
		m_pFont->DrawText(m_pSprite, Players[x].szName, -1, &rectName, DT_SINGLELINE, D3DCOLOR_XRGB(0, 0, 0));
		rectName.left--; rectName.top--;
		m_pFont->DrawText(m_pSprite, Players[x].szName, -1, &rectName, DT_SINGLELINE, Players[x].dwColor);
